...
class UpdateAPIs():
        ...
        def job(self):
            ...
            accounts = EVEAccount.objects.filter(api_last_updated__lt=(datetime.datetime.now() - delta)).exclude(api_status=API_STATUS_ACC_EXPIRED).order_by('api_last_updated')[:self.batches]
